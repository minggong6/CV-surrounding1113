...
import copy
# from utils import read_niigz, write2list, write_image_to_niigz
import cc3d
import glob
import time
# Delay importing heavy/optional libraries that may have incompatible versions
# kimimaro imports cloudvolume internally and may raise ImportError at import time
# (some cloud-volume versions changed their API). We'll import kimimaro lazily
# when we actually need skeletonization to avoid failing at module import.
kimimaro = None
CloudVolume = None
Skeleton = None
try:
    # try to import cloudvolume symbols if available (used in some code paths)
    from cloudvolume import CloudVolume as _CloudVolume
    from cloudvolume import Skeleton as _Skeleton
    CloudVolume = _CloudVolume
    Skeleton = _Skeleton
except Exception:
    # keep them as None; we'll provide a helpful error later if functionality is used
    CloudVolume = None
    Skeleton = None
...
class Node:
    ...
    def __init__(self, instance_segmentation):
        ...
        # Lazy import of kimimaro to avoid import-time failures when cloudvolume
        # or other transitive dependencies are incompatible. If kimimaro is not
        # available, provide a clear message describing how to install it.
        try:
            if kimimaro is None:
                import kimimaro as _kimimaro
            else:
                _kimimaro = kimimaro
        except Exception as e:
            raise ImportError(
                "kimimaro is required for skeletonization but failed to import. "
                "This project expects a compatible combination of 'kimimaro' and 'cloud-volume'. "
                "On Windows it's often easiest to install via conda: "
                "`conda install -c conda-forge kimimaro cloud-volume`\n"
                f"Original error: {e!r}"
            )

        try:
            self.skeleton_0 = _kimimaro.skeletonize(self.segmentation,
                                                   teasar_params={
                                                       'scale': 1,
            ...
            # This often indicates that kimimaro tried to import cloudvolume and
            # failed because of a changed API (for example 'view' symbol missing).
            raise ImportError(
                "Failed to run kimimaro.skeletonize due to an underlying import error. "
                "Common cause: incompatible 'cloud-volume' package version. "
                "Try installing a compatible version via conda: `conda install -c conda-forge kimimaro cloud-volume`\n"
                f"Original error: {e!r}"
            )
        self.skeleton = np.zeros((self.x_size, self.y_size, self.z_size)).astype(np.uint8)

        for idx in self.skeleton_0.keys():
            for vox_list in self.skeleton_0[idx].vertices:
    ...
    # self,keypoint_seg_meandirection()
...
if __name__ == '__main__':
    # filedir = './example'
    # savedir = './save'
    # file_list = glob.glob(filedir + '/*_instance_map.nii.gz')
    # print("file_list",file_list)
    # for file_name in file_list:
    # print("file_name",file_name)
    # instance_segmentation = read_niigz('./example/ME195840_x13824_14080y6912_7168z4140_4230_instance_map.nii')
    # instance_segmentation = read_niigz(file_name)

    # img = sitk.ReadImage("30_seg.nii.gz")
    img = sitk.ReadImage("D:\CV-surrounding1113 (2)\data_p2_origin\seg_68.nii.gz")

    labels = sitk.GetArrayFromImage(img)
    # data = np.load("/mnt/nas/lx/data/Pancreas/multi-phase-pancreas/crop_ven_case_npz/9_ven.npz")
    # labels = data['arr_1']
    print("input labels shape : " + str(labels.shape))
    start_time = time.time()
    image_graph = Node(labels)
    image_graph.compute_graph()
    end_time = time.time()
    print("make_graph_time : ", end_time - start_time)

    segmentation = image_graph.instance_segmentation.copy()
    # segmentation2 = image_graph.instance_segmentation.copy()
    # segmentation3 = image_graph.instance_segmentation.copy()
    ...
    new_nii = sitk.GetImageFromArray(image_instance.astype(np.uint8))
