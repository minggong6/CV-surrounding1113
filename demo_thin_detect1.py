...
for file_dict in file_list:
    ...
    if detect_artery:
        ...
        # skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.5, max_vessel_tumor_dist=1.5, max_min_radis_dist=1, related_range_bias=1)  # 0926(best for CA CHA)
        ...
        # skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.3, max_vessel_tumor_dist=1.5, max_min_radis_dist=1.5, related_range_bias=0.1)  # 0926(best for SMV, strict)
        # skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.3, max_vessel_tumor_dist=1.5, max_min_radis_dist=1, related_range_bias=1)  # AVG SMV
        skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.5, max_vessel_tumor_dist=1.5, max_min_radis_dist=1, related_range_bias=1, avg_radius=True)  # AVG ALL
    ...
    if detect_vein:
        ...
        # skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.5, max_vessel_tumor_dist=1.5,  max_min_radis_dist=1.5)  # 0924
        # skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.3, max_vessel_tumor_dist=1.5, max_min_radis_dist=1.5)  # 0925
        skeleton = tks.Skeleton(img, img_tumor, thin_degree=0.3, max_vessel_tumor_dist=1.5, max_min_radis_dist=1, related_range_bias=1, avg_radius=True)
        skeleton.process_thin_analysis()
        not_found = True
        coordinate_info = ""
        for point in skeleton.thin_point_list:
